...
struct FloydWarshallResult
{
  ...
  FloydWarshallResult(bool nc, std::vector<std::vector<T>> &d, std::vector<std::vector<size_t>> &p)
  {
...
FloydWarshallResult<T> Floyd_Warshall(Graph<T> &graph)
{
  ...
  T INFTY = std::numeric_limits<T>::max();
  bool no_negative_cycle_flag = true;
  ...
  for (size_t k = 0; k < n_verts; k++)
  {
    ...
    for (size_t i = 0; i < n_verts; i++)
    {
      for (size_t j = 0; j < n_verts; j++)
      {
        ...
        if ((*k_minus1_dist)[i][k] == INFTY || (*k_minus1_dist)[k][j] == INFTY || (*k_minus1_dist)[i][j] <= (*k_minus1_dist)[i][k] + (*k_minus1_dist)[k][j])
        {
      ...
      if ((*k_dist)[i][i] < T())
        no_negative_cycle_flag = false;
    }
  }

  return FloydWarshallResult<T>{no_negative_cycle_flag, *k_dist, *k_pred};
}
